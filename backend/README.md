...
```bash
cp .env.example .env
# Fill in: OPENAI_API_KEY (or AZURE_OPENAI_*), OPENROUTER_API_KEY, HF_TOKEN
...
- Lighthouse checkpoints under `./.models/lighthouse/` — generated by
  `backend/video-service/scripts/download_lighthouse_weights.sh`. The
  `video-service` and `video-worker` containers mount this directory at
  `/models/lighthouse:ro` and load CG-DETR (visual MR) + QD-DETR-CLAP
  (audio MR) at startup.

If the checkpoints are missing the API still boots — Lighthouse warm-up logs
a warning and the Ground / Highlights / audio-only paths return empty results
until the weights are in place.
...
Mount the Lighthouse checkpoints at `./.models/lighthouse/` (download with
...
## Encoders inside `video-service`

Frozen-feature encoders live under `backend/video-service/main/encoders/`
(formerly the standalone `jockey/` package). They are imported as
`main.encoders.<module>` and shipped as part of the video-service tree —
nothing else mounts them.

- `main.encoders.indexer` — shot detection, frame extraction
- `main.encoders.viclip_embedder`, `audio_encoder`, `audio_event_encoder`,
  `asr_whisper`, `captioner`, `ocr_encoder`, `moderation_encoder`,
  `search` (text embeddings) — frozen pretrained encoders

Moment retrieval and highlight saliency are NOT in `encoders/` — they come
from the official Lighthouse library, wrapped by
`backend/video-service/main/services/lighthouse_service.py`.
...
| `lighthouse warm-up failed` log on startup                        | Run `bash backend/video-service/scripts/download_lighthouse_weights.sh ./.models/lighthouse` then restart `video-service` |
