...
import { FolderSearch, Search as SearchIcon, X, Zap } from "lucide-react";
...
interface IndexEntry {
  ...
  meta: string;
  videoCount: number;
  /** ISO upload date for real indexes; null for the virtual default index. */
...
export function IndexPicker({ selectedIndexId, onSelect }: Props) {
  ...
  useEffect(() => {
    // The default library index is virtual (no backend row) but should always
    // be offered, mirroring the "My Index (Default)" card on /indexes.
    Promise.all([listIndexes().catch(() => []), listVideos().catch(() => [])])
      .then(([rows, vids]) => {
        const totalS = vids.reduce((s, v) => s + (v.duration_s ?? 0), 0);
        const dur = totalS ? ` (${fmtDuration(totalS)})` : "";
        const defaultEntry: IndexEntry = {
          id: "default",
          title: t("console.indexes.default_title"),
          meta: `${vids.length} ${vids.length === 1 ? "Video" : "Videos"}${dur}`,
        ...
        setEntries([
          defaultEntry,
          ...rows.map((r, n) => asEntry(r, n + 1, t("pgkit.index_picker.untitled"), "Video", "Videos")),
        ]);
      });
  ...
  return (
    <>
      {selected ? (
        // Selected: a compact info badge (title · model · count · created),
        // clickable to reopen the picker. Mirrors the TwelveLabs reference.
        <button
          type="button"
          onClick={() => setOpen(true)}
          title={t("pgkit.index_picker.change")}
          className="w-full rounded-xl border border-[var(--color-chalk)] bg-[var(--color-powder)] px-5 py-3 text-left transition hover:border-[var(--color-slate)]"
        >
          <div className="flex flex-col gap-3">
            <div className="flex h-5 min-w-0 items-center justify-between">
              <p className="truncate text-[13px] text-[var(--color-obsidian)]">{selected.title}</p>
              <span className="inline-flex shrink-0 items-center gap-1">
                <Zap size={15} className="text-[var(--color-gravel)]" />
                <p className="whitespace-nowrap text-[10px] leading-[1.2] text-[var(--color-gravel)]">
                  {t("console.index_wizard.marengo_name")}
                </p>
              </span>
            </div>
            <div className="flex items-center gap-2 text-[10px] leading-[1.2] text-[var(--color-gravel)]">
              <span>
                {t(
                  selected.videoCount === 1
                    ? "console.indexes.video_count_one"
                    : "console.indexes.video_count_other",
                  { count: selected.videoCount },
                )}
              </span>
              {selected.createdAt && (
                <>
                  <div className="h-[14px] w-px bg-[var(--color-chalk)]" />
                  <span>
                    {t("console.indexes.created_on", {
                      date: new Date(selected.createdAt).toLocaleDateString("en-US", {
                        month: "short",
                        day: "2-digit",
                        year: "numeric",
                      }),
